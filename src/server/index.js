...
const AUTO_MOVE_TO_FAILED = String(process.env.AUTO_MOVE_TO_FAILED ?? 'false').toLowerCase() !== 'false';
const KEEP_INPUT = String(process.env.KEEP_INPUT ?? 'false').toLowerCase() !== 'false';
...
const MIN_READY_SIZE = Number(process.env.MIN_READY_SIZE || 1024);
const POLL_TIMEOUT_MS = Number(process.env.POLL_TIMEOUT_MS || 12 * 60 * 1000);
...
function startRunner() {
  ...
  for (const pf of state.files) {
    ...
    for (const f of state.files) {
      if (slots <= 0) break;
      if (f.status === 'pending') {
        const sizeNow = statSync(f.filePath).size;
        if (sizeNow < MIN_READY_SIZE) {
          continue;
        }
        // 移除密钥校验，公开端点无需密钥
        f.status = 'processing';
...
app.get('/docs', (req, res) => { res.sendFile(path.join(DOCS_DIR, 'DETAILED_USAGE.md')); });
app.use('/docs', express.static(DOCS_DIR));
app.get('/api', (req, res) => {
  res.json({ success: true, data: { status: 'ok', endpoints: ['/api/status','/api/stats','/api/health','/api/batch/stats','/api/batch/start','/api/batch/stop','/api/debug/input-files','/api/debug/output-files','/api/files'], docs: '/docs' } });
});

// Upscaler runner control proxy (serve as sub path under main port)
app.use('/upscaler', (await import('http-proxy-middleware')).createProxyMiddleware({
  target: `http://127.0.0.1:${process.env.UPS_CONTROL_PORT || 25349}`,
  changeOrigin: true,
  pathRewrite: { '^/upscaler': '/' }
}));
// --- end api/docs routes ---
// 404 JSON handler (moved to end)
